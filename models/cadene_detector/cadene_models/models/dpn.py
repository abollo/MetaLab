...
__all__ = ['DPN', 'dpn68', 'dpn68b', 'dpn92', 'dpn98', 'dpn131', 'dpn107','cys_dpn']
...
def dpn68(num_classes=1000, pretrained='imagenet'):
    ...
    if pretrained:
        print(pretrained_settings['dpn68'])
...
class InputBlock(nn.Module):
    def __init__(self, num_init_features,input_shape, kernel_size=7,
                 padding=3, activation_fn=nn.ReLU(inplace=True)):
        super(InputBlock, self).__init__()
        input_channel=input_shape[0]
        assert input_channel>=3
        self.conv = nn.Conv2d(
            input_channel, num_init_features, kernel_size=kernel_size, stride=2, padding=padding, bias=False)
        ...
        if self.has_proj:
            ...
            if self.key_stride == 2:
                self.c1x1_w_s2 = BnActConv2d(
                    in_chs=in_chs, out_chs=num_1x1_c + 2 * inc, kernel_size=K_size, stride=2)
            else:
                self.c1x1_w_s1 = BnActConv2d(
                    in_chs=in_chs, out_chs=num_1x1_c + 2 * inc, kernel_size=K_size, stride=1)
        self.c1x1_a = BnActConv2d(in_chs=in_chs, out_chs=num_1x1_a, kernel_size=K_size, stride=1)
        self.c3x3_b = BnActConv2d(
            in_chs=num_1x1_a, out_chs=num_3x3_b, kernel_size=K_size_b,
            stride=self.key_stride, padding=1, groups=groups)
        if b:
            self.c1x1_c = CatBnAct(in_chs=num_3x3_b)
            self.c1x1_c1 = nn.Conv2d(num_3x3_b, num_1x1_c, kernel_size=K_size, bias=False)
            self.c1x1_c2 = nn.Conv2d(num_3x3_b, inc, kernel_size=K_size, bias=False)
        else:
            self.c1x1_c = BnActConv2d(in_chs=num_3x3_b, out_chs=num_1x1_c + inc, kernel_size=K_size, stride=1)
...
class DPN(nn.Module):
    def __init__(self, small=False, num_init_features=64, k_r=96, groups=32,
                 ...
                 num_classes=1000,input_shape=[3,224,224], test_time_pool=False):
        ...
        # conv1
        if small:
            blocks['conv1_1'] = InputBlock(num_init_features,input_shape, kernel_size=3, padding=1)
        else:
            blocks['conv1_1'] = InputBlock(num_init_features, input_shape,kernel_size=7, padding=3)
            #blocks['conv1_1'] = InputBlock(num_init_features, input_shape,kernel_size=9, padding=3) #似乎9不明显
    ...
    def logits(self, features):
        input_ld = self.input_shape[1]
        assert input_ld>32
        if not self.training and self.test_time_pool:
            k_size = input_ld//32
            #x = F.avg_pool2d(features, kernel_size=7, stride=1)        #cys        3/31/2019
            x = F.avg_pool2d(features, kernel_size=k_size, stride=1)
            out = self.last_linear(x)
            # The extra test time pool should be pooling an img_size//32 - 6 size patch
            out = adaptive_avgmax_pool2d(out, pool_type='avgmax')
            if hasattr(self, 'gbdt_features'):
                feats = x.cpu().detach().numpy()
                feats = feats.squeeze()
                self.gbdt_features.append(feats)
        else:
...
class cys_dpn(DPN):
    def __init__(self,num_classes,input_shape, pretrained='cys_dpn_at___'):
        # small=True,
        super(cys_dpn, self).__init__( num_init_features=32, k_r=128, groups=32,
                         k_sec=(3, 4, 12, 3), inc_sec=(16, 32, 32, 64),num_classes=num_classes,input_shape=input_shape,test_time_pool=True
                         )
        '''model = DPN(
            small=True, num_init_features=10, k_r=128, groups=32,
            k_sec=(3, 4, 12, 3), inc_sec=(16, 32, 32, 64),
